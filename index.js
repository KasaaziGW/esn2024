...
app.get("/citizens", (request, response) => {
  const session = request.session;
  
  // Check if the user exists (if not, redirect to the login page)
  if (!session.uid || !session.fname) {
   // request.flash("error", "You must be logged in to view this page.");
  ...
  Citizen.find({})
    .then((citizens) => {
      // Render the citizens page with retrieved data
      response.render("citizens", {
        data: {
          userid: session.uid,
          fullname: session.fname,
          citizens: citizens
    ...
    .catch((err) => {
      ...
      request.flash("error", `Cannot Find Citizens! \n${err}`);
      response.redirect("/");
    });
});
